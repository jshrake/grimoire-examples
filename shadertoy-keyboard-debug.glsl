// Original source: https://www.shadertoy.com/view/4dGyDm
/*
[font]
...
[[pass]]
iChannel0 = {resource="font", filter="linear"}
iChannel1 = {resource="keyboard", filter="linear"}
*/
...
vec3 sample_grad_dist(vec2 uv, float font_size) {
  ...
  vec3 grad_dist =
      (textureLod(iChannel0, uv, 0.).yzw - FONT_TEX_BIAS) * font_size;
  ...
  if (dbox < 0.) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (digit > 0 || nonzero || i == 2) {
        ...
        dtext =
            min(dtext, max(max(dbox.x, dbox.y), sample_grad_dist(uv, 1.0).z));
        nonzero = true;
      }
    }

    vec3 textcolor =
        ktex[2] ? vec3(0) : (ktex[1] || ktex[0]) ? vec3(1) : vec3(0.9);
